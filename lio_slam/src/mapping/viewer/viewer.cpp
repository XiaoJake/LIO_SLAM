...
bool Viewer::SaveMap() {
    ...
    if (global_map_ptr->points.size() > 10000) {
        std::shared_ptr<VoxelFilter> map_filter_ptr = std::make_shared<VoxelFilter>(0.05, 0.05, 0.05);
    ...
    std::cout << std::endl << map_path_ << std::endl << std::endl;
...
bool Viewer::GetLocalMap(CloudData::CLOUD_PTR& local_map_ptr) {
    ...
    //local_map_filter_ptr_->Filter(local_map_ptr, local_map_ptr);// ??? 这里如果再滤一次会出现很多散点 本不该有的散点
    return true;
}
